		...
		<div className='space-y-6'>
			...
			<RadioGroup value={selectedTrack || ''} onValueChange={setSelectedTrack} className='space-y-4'>
				{SAMPLE_TRACKS.map((track) => (
					<Card
						key={track.id}
						className={`cursor-pointer p-4 transition-colors ${selectedTrack === track.id ? 'border-primary' : ''}`}
					>
						<div className='flex items-start space-x-4'>
							...
							>
								{playingTrack === track.id ? <PauseCircle className='h-6 w-6' /> : <PlayCircle className='h-6 w-6' />}
							</Button>
							{playingTrack === track.id && (
								<AudioPlayer url={track.url} isPlaying={true} onToggle={() => setPlayingTrack(null)} />
							)}
						</div>
					</Card>
				))}
